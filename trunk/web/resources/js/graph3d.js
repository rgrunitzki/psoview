...
links.Graph3d = function (container) {
    ...
    this.showAxes = true;  // Ricardo Grunitzki
    ...
    this.eye = new links.Point3d(0, 0, -0.7);  // TODO: set eye.z about 3/4 of the width of the window?

    this.dataTable = null;  // The original data table
    this.dataPoints = null; // The table with point objects
    
    this.dataTable2 = null;  // The original data table
    this.dataPoints2 = null; // The table with point objects
...
links.Graph3d.Camera.prototype.setArmRotation = function(horizontal, vertical) {
    ...
    //          Estas linhas limita os movimentos do gráfico
 ...
 * 
 * @param {google.visualization.DataTable} points Contêm os pontos que serão plotados no gráfico.
 */
links.Graph3d.prototype.draw = function(data, options, points) {
...
links.Graph3d.prototype._getStyleName = function(styleNumber) {
    switch (styleNumber) {
        case links.Graph3d.STYLE.DOT:
            return "dot";
        case links.Graph3d.STYLE.DOTLINE:
            return "dot-line";
        case links.Graph3d.STYLE.DOTCOLOR:
            return "dot-color";
        case links.Graph3d.STYLE.DOTSIZE:
            return "dot-size";
        case links.Graph3d.STYLE.LINE:
            return "line";
        case links.Graph3d.STYLE.GRID:
            return "grid";
        case links.Graph3d.STYLE.SURFACE:
            return "surface";
    }
...
links.Graph3d.prototype._dataInitialize2 = function (data, style) {
    if (data === undefined || data.getNumberOfRows === undefined)
        return;
    ...
    this.dataFilter2 = undefined;

    // check if a filter column is provided
    if (this.colFilter && data.getNumberOfColumns() >= this.colFilter) {
        if (this.dataFilter2 === undefined) {
            this.dataFilter2 = new links.Filter(data, this.colFilter, this);

            var me = this;
            this.dataFilter2.setOnLoadCallback(function() {
                me.redraw();
            });
        }
    }
...
links.Graph3d.prototype._pointsInitialize = function (data, style) {
    if (data === undefined || data.getNumberOfRows === undefined)
        return;
...
links.Graph3d.prototype._readData2 = function(data) {
    // read the data
    this._dataInitialize2(data, this.style);

    if (this.dataFilter2) {
        // apply filtering
        this.dataPoints2 = this.dataFilter._getDataPoints();
    }
    else {
        // no filtering. load all data
        this.dataPoints2 = this._getDataPoints(this.dataTable2);
    }
 ...
 * @param {google.visualization.DataTable} points    Optional, novos pontos
 */
links.Graph3d.prototype.redraw = function(data, points) {
    // load the data if needed
    if (data !== undefined) {
        this._readData(data);
    }
    this._readData2(points);
 ...
 * This function can be used when the style is "grid" this._redrawDataDot();
 */
...
links.Graph3d.prototype._redrawDataDot = function() {
    ...
    if (this.dataPoints2 === undefined || this.dataPoints2.length <= 0)
        return;  // TODO: throw exception?

    // calculate the translations of all points
    for (var i = 0; i < this.dataPoints2.length; i++) {
        var trans = this._convertPointToTranslation(this.dataPoints2[i].point);
        var screen = this._convertTranslationToScreen(trans);

        this.dataPoints2[i].trans = trans;
        this.dataPoints2[i].screen = screen;
    }
    ...
    this.dataPoints2.sort(sortDepth);

    // draw the datapoints as colored circles
    var dotSize = this.frame.clientWidth * 0.02;  // px
    for (var i = 0; i < this.dataPoints2.length; i++) {
        var point = this.dataPoints2[i];
        ...
        var reflection = radius*0.2/4;
        ctx.save();
        size = 20;
        ctx.translate(point.screen.x, point.screen.y);
        var radgrad = ctx.createRadialGradient(-reflection,-reflection,reflection,0,0,radius);

        radgrad.addColorStop(0.1, '#FFFFFF');
        radgrad.addColorStop(0.5, "#DB4865");
        radgrad.addColorStop(0.6, 'rgba(1,159,98,0)');
